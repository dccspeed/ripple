...
class SuperEmbedding {
   private :
	...
	std::vector<double> degreeEmbeddings;
  	std::unordered_map<earray<int>,int, earrayhash<int>> emb2index;
   ...
   public :
	...
	std::vector<size_t> getSubsetWordsHash(T &embedding);

	friend std::ostream& operator<<(std::ostream& os, SuperEmbedding& se) {
		std::vector<T> &embs = se.getEmbeddings();
		os << "SuperEmbedding: { size: " << embs.size() << " Total InDegree: " << se.getInternalDegree() << " Total Degree: " << se.getExternalDegree() <<" emb2idx size: " << se.emb2index.size() << " } "<< std::endl;
