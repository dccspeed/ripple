...
struct Graph {
	...
	public:
		...
		void sortNeighborhoodIdx(std::vector<uint> &, std::vector<uint> &);
		...
		bool isHighDegreeNode(int, double);
		...
		int getDegreeOfTop(double);
		std::vector<int> &getNodesCoreness();	
		std::vector<int> &getNodesCorenessInverse();	
		std::vector<int> &getNodesColors(int);	
		...
		std::vector<uint> getNaiveCodeHashValueVector();
		size_t getBlissCodeHashValue();

		void colorNodes(int);
		int getRandomNodeBiased();
		int getRandomNodeNeighbor(int);
		int getRandomNodeNeighbor(int, uint, uint);
		...
		std::vector<int> &getHopDistNodes(boost::dynamic_bitset<> &);
		std::vector<int> &getHopDistNodes(std::unordered_set<int> &);
		std::pair<int, int> getNumEdgesWithinNodes(std::unordered_set<int> &);
