    ...
    while( std::getline(is_file, line) )
    {
        ...
        if( std::getline(is_line, key, '=') )
        {
            ...
            if( std::getline(is_line, value))
                configMap.insert(std::make_pair(strip(key), strip(value)));
        }
    }
};

void Config::load(std::string confFile, std::string confString) {
    load(confFile);
    if (!confString.empty()){
        std::vector<std::string> tokens;
        boost::split(tokens, confString, boost::is_any_of("@"));
        for (std::string& token : tokens) {
            std::vector<std::string> kvPair;
            boost::split(kvPair, token, boost::is_any_of("="));
            configMap[strip(kvPair[0])]= strip(kvPair[1]);
        }
    }
};
...
uint Config::getKeyAsUint(std::string key) {
	StrStrMap::iterator it = Config::configMap.find(key);
	return std::stoul (it->second,nullptr);
};
...
bool Config::existKey(std::string key) {
	StrStrMap::iterator it = configMap.find(strip(key));
	
	if (it == configMap.end())  {
		std::cout << "warning: key " << strip(key) << " not found! using default value!" << std::endl;
		//print();
	}
...
void setStackSize() {
    ...
    if (result == 0)
    {
        ...
        {
			rl.rlim_cur = std::min(kStackSize, rl.rlim_max);
...
bool isPrime(Uint n) {
    ...
    if (n % 2 == 0){return false;}
    // rule out odds up to the square root of n
    for (int i = 3, end = (Uint)sqrt((float)n);i <= end; i += 2)
    {
            if (n % i == 0) {return false;}
    }
    return true;
