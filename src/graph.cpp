...
int Graph::getLabelOfNodeAt(int i) {
	return nodes[i].getLabel();
}
...
bool Graph::isHighDegreeNode(int i, double p) {
       double pi = (double) (i+1) / (double) getNumberOfNodes();
       if (pi >= p) {
		return true;
	}
	return false;
...
void Graph::createNeighborhoodIndex() {
    ...
    neighborhoodEdge.resize(edges.size(), NeighborhoodSet());
	...
	std::stable_sort(neighborhoodIdx[i].begin(), neighborhoodIdx[i].end());
...
void Graph::sortNeighborhoodIdx(std::vector<uint> &value_vec, std::vector<uint> &value_vec2) {
	for (int i = 0; i < this->getNumberOfNodes(); i++) {
		std::stable_sort(neighborhoodIdx[i].begin(), neighborhoodIdx[i].end(), VectorCompWithOthers(value_vec, value_vec2)); 
	}
}
...
Graph Graph::swapNodeLabelsRandom(double p, int vl) {
	...
	pos = Randness::getInstance()->get_a_random_number(0,nodes.size());
    ...
    for (std::set<int>::iterator it=positions.begin(); it !=positions.end(); it++){
        ...
        while (newLabel==currentLabel){
            newLabel = Randness::getInstance()->get_a_random_number(0,vl);
        }
...
Graph Graph::removeEdgesRandom(double p){
	...
	int pos = Randness::getInstance()->get_a_random_number(0,edges.size());
...
void Graph::modifyByAddingNodes(int n, double p) {
	...
	for (int i = 0; i < n; i++) {
		int l = Randness::getInstance()->get_a_random_number(0,nlabel2qtd.size());
		//adding edges
		for (int j = 0; j < (int) nodes.size(); j++) {
			if (Randness::getInstance()->random_uni01() < p) {
...
std::vector<int> &Graph::getNodesDegrees() {
	degrees.resize(getNumberOfNodes());
...
int Graph::getDegreeOfTop(double p) {
	std::vector<std::pair<int, int>> degreesSorted(getNumberOfNodes());
	for (int i = 0; i < getNumberOfNodes(); i++) {
		degreesSorted[i].first = i;
		degreesSorted[i].second = getDegreeOfNodeAt(i);
	}

	std::stable_sort(degreesSorted.begin(), degreesSorted.end(), [](auto &left, auto &right) {
		return left.second < right.second;
	});
	
	int i =  std::floor((double)getNumberOfNodes()*(1.-p)); 
	std::cout << "node id of the ith highest degree: " << degreesSorted[i].first << " degree: " <<  degreesSorted[i].second << std::endl;

	return degreesSorted[i].second;
}

std::vector<int> &Graph::getNodesColors(int k) {
	colorNodes(k);
	return colors;
}
...
std::vector<int> &Graph::getNodesCorenessInverse() {
        ...
        for (int k = getLargestNodeDegree(); k>=0;  k--) {
                //std::cout << "getting nodes with degree: " << k << std::endl;
                while (!D[k].empty()) {
                        int i = *(D[k].begin());
                        //std::cout << "=checking node " << i << std::endl;
                        D[k].erase(i);
                        coreness[i] = getLargestNodeDegree()-k;
                        std::vector<int> neighs = getNeighborhoodIdxVertexOfVertexAt(i);
                        for (int j : neighs) {
                                if (d[j] == -1) {
                                        D[d[j]].erase(j);
...
std::vector<uint> Graph::getNaiveCodeHashValueVector() {
        std::vector<uint> hash(2*getNumberOfNodes(),0);
        ...
        for (Edge e : edges) {
                //including node src and dest
                Node src = this->getNodeAt(e.getFromNodeId());
                Node dest = this->getNodeAt(e.getToNodeId());
                hash[e.getFromNodeId()] |= 1 << (e.getToNodeId()+1);
                hash[e.getToNodeId()] |= 1 << (e.getFromNodeId()+1);
...
int Graph::getRandomNodeNeighbor(int v) {
	return *Randness::getInstance()->random_element(neighborhoodIdx[v].begin(), neighborhoodIdx[v].end());	
}

int Graph::getRandomNodeNeighbor(int v, uint idx, uint offset) {
	return *Randness::getInstance()->random_element(neighborhoodIdx[v].begin()+idx, neighborhoodIdx[v].begin()+idx+offset);	
}
...
void Graph::colorNodes(int c) {
	...
	for (int i = 0; i < getNumberOfNodes(); i++)
		colors[i] = Randness::getInstance()->get_a_random_number(0,c);
}
...
std::vector<int> Graph::getConnectedNodes(std::unordered_set<int> &nodes, int k) {
	...
	while ((int)reach.size()!=k && !q.empty()) {
		int top  = q.top();
		q.pop();
...
//get distance vector from set
std::vector<int> &Graph::getHopDistNodes(boost::dynamic_bitset<> &bs) {
        dists.resize(getNumberOfNodes());
...
//get distance vector from set
std::vector<int> &Graph::getHopDistNodes(std::unordered_set<int> &s) {
        dists.resize(getNumberOfNodes());
	std::fill(dists.begin(), dists.end(), -1);
        std::queue<size_t> q;
        ...
        while (!q.empty()) {
                ...
                q.pop();
                std::vector<int> &neighs = getNeighborhoodIdxVertexOfVertexAt(top);
		//std::cout  << "top dists[" << top << "]: " << dists[top] << std::endl;
                for (int n : neighs){
                        if (dists[n]<0) {
                                dists[n]=dists[top]+1;
                                q.push(n);
...
//get distance vector from set
std::pair<int,int> Graph::getNumEdgesWithinNodes(std::unordered_set<int> &s) {
        dists.resize(getNumberOfNodes());
        std::fill(dists.begin(), dists.end(), -1);
        std::queue<size_t> q;
	...
	int maxDegree = 0;
        ...
        return std::pair<int, int> (numEdges/2, maxDegree);
}
