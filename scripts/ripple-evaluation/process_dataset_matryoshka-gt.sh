...
do
		...
		do
			...
			grep  "==>update final pattern " ${LOG_DIR}/*-${DATASET}.$k-*.log | cut -f 4 -d " " | sort -n | uniq > $pattfile
			echo "grep  \"==>update final pattern \" ${LOG_DIR}/*-${DATASET}.$k-*.log | cut -f 4 -d \" \" | sort -n | uniq > $pattfile"
